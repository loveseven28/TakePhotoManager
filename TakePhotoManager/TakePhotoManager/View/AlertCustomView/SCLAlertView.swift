...
open class SCLAlertView: UIViewController {
	...
	let kTitleMinimumScaleFactor: CGFloat
        ...
        public struct Margin {
          ...
          public init(titleTop: CGFloat = 10,
        ...
        case .horizontal:
            ...
            if numberOfButton == 1 {
                if let btn = buttons.first {
                    let x = (appearance.kWindowWidth / 2 ) - 60
                    btn.frame = CGRect(x:x, y:y, width: 120, height:appearance.kButtonHeight)
                    btn.layer.cornerRadius = appearance.buttonCornerRadius
                }
            }else {
                let  buttonsSpace = CGFloat(10) * (numberOfButton - 1)
